...
- 规则版报告。
- 同步 AI 出题 + 规则出题兜底。
- Skill 资源加载、分类优先级配额、Persona/分类 Prompt 注入。
- AI 失败时按照当前 Skill 分类和配额生成规则兜底题。
- AI 单题答案评估 + 规则评估兜底。
- 报告展示评分、反馈、参考答案和关键点。
- 服务拆分：会话、历史、报告。
- 题目答案聚合工具。
...
1. 会话持久化 `skillId`，修正多 Skill 下未完成会话复用逻辑。
2. 增加第二个 Skill 和前端面试方向选择。
3. 增加历史题目去重和 AI 结构化输出重试。
...
- Skill 出题核心第一阶段已经完成，包括资源加载、分类配额、Prompt 注入和 Skill 感知规则兜底。
- 当前继续完成会话 `skillId` 持久化、第二个 Skill 和前端方向选择。
- 保留规则版出题和规则版报告作为兜底，稳定后再异步化。
