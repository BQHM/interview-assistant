...
`interview-assistant` 当前已经完成后端 MVP 的两条核心主链路：简历处理和文字模拟面试。文字面试模块已经推进到历史查询、独立答案表、报告聚合、同步 AI 出题、AI 单题评估入库，并完成 Skill 出题核心第一阶段和会话级 `skillId` 闭环：资源化 Skill 加载、分类优先级配额、Persona/分类 Prompt 注入、Skill 感知规则兜底、会话持久化和按方向复用。前端核心页面也已通过浏览器主链路验收。项目还未完成第二个 Skill、题目去重和异步化，但已经具备面试演示价值。

| 维度 | 当前完成度 | 判断 |
| --- | --- | --- |
| 按当前 README 定义的后端 MVP | 约 95% | 简历与文字面试主流程、历史、答案表、同步 AI 出题、AI 单题评估入库、报告读取评分/反馈/参考答案/关键点已完成手工验收 |
| 按当前前端核心主链路 | 约 90% | React + TypeScript 前端已覆盖简历管理、上传、详情、面试中心、答题、记录和报告；已完成浏览器端主链路验收，面试记录页已区分继续面试和查看报告，简历详情页已展示完整 AI 分析结果 |
| 按 `interview-guide` 完整平台 | 约 45% | 参考项目还包含 Redis、RAG、日程、语音、PDF、Docker 等能力；当前已完成核心展示链路 |
...
| 复用未完成会话 | 已完成 | 按 `resumeId + skillId + status` 返回同方向最近一条未完成会话 |
| 规则版出题 | 已完成 Skill 感知版 | 按当前 Skill 分类、优先级和题量配额生成兜底题，并区分有无简历的提问上下文 |
| 同步 AI 出题 | 已完成 Skill 驱动第一阶段 | Persona、分类和目标题量进入 Prompt，失败时使用同一 Skill 的规则兜底 |
| Skill 配置加载 | 已完成第一阶段 | 从 `SKILL.md` 和 `skill.meta.yml` 加载 `java-backend`，支持非法 Skill 校验 |
| 分类题量分配 | 已完成 | 支持 `ALWAYS_ONE`、`CORE`、`NORMAL` 和剩余题量轮询 |
...
| 参考项目能力 | 当前状态 | 后续建议 |
| --- | --- | --- |
| 报告展示参考答案和关键点 | 已完成基础版 | 当前报告 DTO 已支持返回 `referenceAnswer` 和 `keyPoints`，并已通过主链路验收 |
| Skill 驱动出题 | 会话级闭环完成 | 已持久化会话 `skillId`，下一步增加第二个 Skill 和前端选择 |
...
| 能力 | 当前状态 | 后续建议 |
| --- | --- | --- |
| 单元测试和集成测试 | 部分完成 | Skill 加载和出题相关 12 个测试已通过，仍需继续补 Controller 和集成测试 |
...
| 前端 | 核心页面第一版已完成 | 已创建 Vite + React + TypeScript 工程，并完成简历管理、上传、详情、面试中心、文字面试、面试记录和报告页 |
...
| interview | 完整 | MVP 已完成，Skill 出题和会话级 `skillId` 闭环已完成 | 增加第二个 Skill 后，再做题目去重和 Redis 异步化 |
| frontend | 完整 | 核心页面第一版已完成，体验补强中 | 先补空状态、错误提示和时间格式展示，再考虑更完整交互 |
...
1. 增加第二个 Skill 和前端面试方向选择。
2. 补充历史题目去重和更稳定的 AI 结构化输出。
3. Redis 会话缓存和接口限流。
4. Redis Stream 异步化简历分析和面试评估。
5. PDF 导出。
6. 知识库 / RAG、面试日程和语音面试。
...
| 已接入同步 Skill AI 出题 | 创建面试接口仍受 AI 耗时和输出稳定性影响 | 已保留 Skill 感知规则兜底，后续增加题目去重和异步/缓存策略 |
...
| 前端复制参考项目过深 | 学习成本上升且容易引入未完成模块 | 当前已只复刻核心布局和已完成业务页面，不提前引入知识库、日程和语音 |
| 当前 Git 状态有迁移残留 | 可能误提交旧路径 | 正式开发前整理 `git status` |

## 7. 近期推荐结论

下一阶段不要马上做 Redis、RAG 或语音面试。Skill 已经从“出题时可用”推进到“会话全生命周期可追踪”，现在应验证多方向扩展能力：

1. 增加第二个 Skill，并验证加载、分类配额、AI Prompt 和规则兜底。
2. 在前端增加面试方向选择，验证不同 Skill 可以创建独立会话。
3. 再增加历史题去重和结构化输出重试，之后考虑 Redis Stream 异步化。
