...
| 按当前前端核心主链路 | 约 85% | React + TypeScript 前端已覆盖简历管理、上传、详情、面试中心、答题、记录和报告；已完成浏览器端主链路验收，仍需完善简历分析展示和继续面试入口 |
...
| frontend | 完整 | 核心页面第一版已完成，体验补强中 | 先补简历分析展示和继续面试入口，再考虑更完整交互 |
...
1. 完善前端体验：简历详情展示完整 AI 分析结果，面试记录页区分继续面试和查看报告。
...
1. 简历详情页补齐 AI 分析结果展示。
2. 面试记录页增加未完成面试的“继续面试”入口。
3. 补核心 Service 测试。
4. 再将同步 AI 出题演进为 Skill 驱动出题，之后考虑 Redis Stream 异步化。
