...
`interview-assistant` 当前已经完成后端 MVP 的两条核心主链路：简历处理和文字模拟面试。文字面试模块已经推进到历史查询、独立答案表、报告聚合、同步 AI 出题、AI 单题评估入库，并完成 Skill 出题核心第一阶段：资源化 Skill 加载、分类优先级配额、Persona/分类 Prompt 注入和 Skill 感知规则兜底。前端核心页面也已通过浏览器主链路验收。项目还未完成会话 `skillId` 持久化、第二个 Skill、题目去重和异步化，但已经具备面试演示价值。
...
| Skill 驱动出题 | 核心第一阶段完成 | 下一步持久化会话 `skillId`、增加第二个 Skill 和前端选择 |
...
| interview | 完整 | MVP 已完成，Skill 出题核心第一阶段已完成 | 先完成会话 `skillId` 闭环，再做题目去重和 Redis 异步化 |
...
1. 会话持久化 `skillId`，按 `resumeId + skillId + status` 复用未完成会话。
2. 增加第二个 Skill 和前端面试方向选择。
3. 补充历史题目去重和更稳定的 AI 结构化输出。
4. Redis 会话缓存和接口限流。
...
下一阶段不要马上做 Redis、RAG 或语音面试。当前最重要的是把 Skill 从“出题时可用”推进到“会话全生命周期可追踪”：

1. 在会话实体、DTO、历史列表和复用查询中加入 `skillId`。
2. 增加第二个 Skill 和前端方向选择，验证扩展性。
