...
`interview-assistant` 当前已经完成后端 MVP 的两条核心主链路：简历处理和文字模拟面试。文字面试模块已经从单纯跑通流程，推进到历史查询、独立答案表、报告聚合、同步 AI 出题、AI 单题评估入库，以及报告读取评分、反馈、参考答案和关键点阶段。前端核心页面也已完成第一版，并已通过浏览器完成“简历管理 -> 创建面试 -> 答题 -> 查看报告 -> 面试记录查看报告”的主链路验收。它还不是完整平台，但已经具备面试演示价值和继续向 `interview-guide` 工程化能力演进的基础。
...
| interview | 完整 | MVP 已完成并进入测试补强和 Skill 出题准备阶段 | 先补核心 Service 测试，再做 Skill 出题和 Redis 异步化 |
...
1. 完善前端体验：补充空状态、错误提示和时间格式展示。
2. 后端核心 Service 测试补强，优先覆盖 AI 失败兜底、提交答案推进和报告聚合。
3. Skill 驱动 AI 出题，补充题目去重和更稳定的结构化输出。
4. Redis 会话缓存和接口限流。
...
下一阶段不要马上做 Redis、RAG 或语音面试。当前后端主链路已经通过 Postman 验收，前端核心主链路也已通过浏览器验收，最适合当前学习和面试展示的是先补前端体验和测试：

1. 统一前端空状态、错误提示和时间格式展示。
2. 补核心 Service 测试。
3. 再将同步 AI 出题演进为 Skill 驱动出题，之后考虑 Redis Stream 异步化。
