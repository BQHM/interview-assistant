...
- 当前阶段：简历模块和文字面试后端主链路已完成；AI 出题已完成 Skill 资源加载、分类配额、Persona/分类 Prompt 注入和 Skill 感知规则兜底；AI 单题评估仍采用同步调用并保留规则兜底；前端核心页面已通过浏览器验收。
- 当前优先级：完成会话 `skillId` 持久化和多 Skill 会话复用闭环，再增加第二个 Skill、前端方向选择、题目去重和结构化输出重试。
