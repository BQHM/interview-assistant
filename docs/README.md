...
- 当前阶段：简历模块和文字面试后端主链路已完成，面试历史、独立答案表、历史详情/报告聚合和服务拆分已完成；AI 出题和 AI 单题答案评估已接入同步链路，并保留规则兜底；前端核心页面已完成第一版，前后端主链路已通过浏览器验收。
- 当前优先级：先完善前端核心体验，例如简历详情展示完整 AI 分析结果、面试记录区分继续面试和查看报告；随后补强测试、Skill 出题、Redis、RAG 等能力。
