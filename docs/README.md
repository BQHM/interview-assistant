...
- 当前阶段：简历模块和文字面试后端主链路已完成，面试历史、独立答案表、历史详情/报告聚合和服务拆分已完成，AI 单题答案评估已接入提交答案链路，报告已优先读取答案表中的评分和反馈。
- 当前优先级：先完成报告接口展示参考答案和关键点列表，并手工验证提交答案、评估入库、报告读取评估结果的完整链路；之后再进入 AI 出题、Redis、RAG 和前端。
