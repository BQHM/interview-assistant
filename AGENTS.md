...
当前项目已开始引入前端。前端当前目标不是差异化设计，而是先以 `interview-guide` 为目标完成等价复刻，后续项目完整后再考虑产品和视觉差异化：
...
前端实现应先复刻参考项目的管理台式结构：左侧导航栏 + 右侧业务内容区，按 `api/`、`types/`、`components/`、`pages/` 分层组织代码。

第一阶段只实现已完成后端主链路对应页面：简历管理、简历上传、简历详情、面试中心、文字面试、面试记录和面试报告。暂不做大横幅首页、聊天窗口式改造、知识库、日程、语音面试和差异化 UI。
...
6. 前端页面：先对齐 `interview-guide` 的左侧导航 + 内容区结构，完成简历、文字面试、面试记录核心页面。
7. Redis 能力：限流、会话缓存、Redis Stream 异步分析/评估。
8. 知识库与 RAG：文档上传、切分、向量化、检索问答、SSE。
9. 面试日程：邀请解析、日历视图、状态流转、提醒。
